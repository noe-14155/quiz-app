...
def submit_chill_answer(payload: ChillAnswerPayload, user=Depends(get_current_user_optional)):
    ...
    # Comparaison par le TEXTE de la réponse, pas par sa position : /questions
    # et /answer mélangent chacun indépendamment, donc la position vue par le
    # joueur ne correspond pas forcément à celle relue ici. Le texte, lui,
    # reste le même quel que soit l'ordre d'affichage.
    correct_text = question["choix"][question["bonne_reponse"] - 1]
    correct = payload.answer_text == correct_text
    ...
    return {"correct": correct, "explication": question["explication"], "bonne_reponse_text": correct_text}
